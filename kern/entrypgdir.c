...
// entry.S 页目录，将虚拟地址 KERNBASE开始的第一个 4MB地址空间 [KERNBASE, KERNBASE+4MB)，
// 映射到实际物理地址 [0, 4MB)，4MB地址空间具有1024个4KB页表，足够OS启动使用
// 同时，虚拟地址的 [0, 4MB)页映射到物理地址 [0, 4MB)，
// 这个区域对 entry.S 中几条指令非常重要，然后我们不会再使用它。

// 页目录（和页表），必须启动一个页的边界，因此“用__aligned__”属性；
// 也正因此，受到链接及静态初始化的限制，这里我们用“X + PREP”，而不是更标准的“X | PREP”；
// 在其他地方，应该使用“|”组合标志。

__attribute__((__aligned__(PGSIZE)))
pde_t entry_pgdir[NPTENTRIES] = {
	// 映射 虚拟地址 [0, 4MB) 到 物理地址 [0, 4MB)
	[0] 
		= ((uintptr_t)entry_pgtable - KERNBASE) + PTE_P,
	// 映射 虚拟地址 [KERNBASE, KERNBASE+4MB) 到 物理地址 [0, 4MB)
	[KERNBASE >> PDXSHIFT] 
		= ((uintptr_t)entry_pgtable - KERNBASE) + PTE_P + PTE_W
};

// 页表条目0映射到物理页面0，项1到物理页面1，等等
